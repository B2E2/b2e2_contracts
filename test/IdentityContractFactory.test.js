...
contract('IdentityContractFactory', function(accounts) {
  ...
  it("can create Identity Contracts.", async function() {
	...
	assert.equal(idcDeployment1.logs[1].event, 'IdentityContractCreation');
	assert.equal(idcDeployment1Twin.logs[1].event, 'IdentityContractCreation');
	assert.equal(idcDeployment2.logs[1].event, 'IdentityContractCreation');

	// Make sure that the creation events have been emitted and that there are no duplicate addresses.
	let idcAddress1 = idcDeployment1.logs[1].args.idcAddress;
	let idcAddress1Twin = idcDeployment1Twin.logs[1].args.idcAddress;
	let idcAddress2 = idcDeployment2.logs[1].args.idcAddress;
	...
	let owner1 = idcDeployment1.logs[1].args.owner;
	let owner1Twin = idcDeployment1Twin.logs[1].args.owner;
	let owner2 = idcDeployment2.logs[1].args.owner;
  ...
  it("knows its registered contracts.", async function() {
	...
	let idcThroughFactoryAdress = idcThroughFactory.logs[1].args.idcAddress;

	let independentIdc = await IdentityContract.new(marketAuthority.address, 0, {from: accounts[2]});
